...
get eli scores for alignment graph edges
        ...
        if len(path) == 3:
            ...
            for key, value in edge_weight_dict.items():
                if ((path[0], path[1]) == key) or ((path[1], path[0]) == key):
                    edge1_weight = value
                elif ((path[1], path[2]) == key) or ((path[2], path[1]) == key):
                    edge2_weight = value
...
# nx.write_gml(alignment_graph, "AlignNemo/alignment_graph.gml", stringizer=lambda x: str(x))
